...
func determineEnvironment() conftypes.RunInEnvironment {
	...
	_, err := os.Stat("../../cmd/elektractl/elektractl.env")
	if err != nil {
		log.Info().Msg("Config Manager: File .env not found, RunningInKubernetes inferred")
		return conftypes.RunningInK8s
	}
	log.Info().Msg("Config Manager: File .env found, RunningInDocker inferred")
	err = godotenv.Load("../../cmd/elektractl/elektractl.env")
	if err != nil {
		log.Info().Str("err", err.Error()).Msg("Config Manager: File .env not loaded")
	}
