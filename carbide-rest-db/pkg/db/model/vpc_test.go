...
func TestVpcSQLDAO_GetByID(t *testing.T) {
	...
	vpc := testBuildVpc(t, dbSession, nil, "test-vpc", nil, tn.Org, ip.ID, tn.ID, st.ID, db.GetStrPtr(VpcEthernetVirtualizer), db.GetUUIDPtr(uuid.New()), nil, db.GetStrPtr(VpcStatusReady), tnu.ID, &networkSecurityGroup.ID)
...
func TestVpcSQLDAO_GetCountByStatus(t *testing.T) {
	...
	vpc := testBuildVpc(t, dbSession, nil, "test-vpc", nil, tn1.Org, ip.ID, tn1.ID, st.ID, db.GetStrPtr(VpcEthernetVirtualizer), db.GetUUIDPtr(uuid.New()), nil, db.GetStrPtr(VpcStatusReady), tnu1.ID, nil)
	assert.NotNil(t, vpc)
	vpc2 := testBuildVpc(t, dbSession, nil, "test-vpc-1", nil, tn1.Org, ip.ID, tn1.ID, st.ID, db.GetStrPtr(VpcEthernetVirtualizer), db.GetUUIDPtr(uuid.New()), nil, db.GetStrPtr(VpcStatusDeleting), tnu1.ID, nil)
	assert.NotNil(t, vpc2)
	vpc3 := testBuildVpc(t, dbSession, nil, "test-vpc-1", nil, tn1.Org, ip.ID, tn1.ID, st.ID, db.GetStrPtr(VpcEthernetVirtualizer), db.GetUUIDPtr(uuid.New()), nil, db.GetStrPtr(VpcStatusReady), tnu1.ID, nil)
...
func TestVpc_GetAll(t *testing.T) {
	...
	for i := 0; i < totalCount; i++ {
		...
		if i%2 == 0 {
			vpc = testBuildVpc(t, dbSession, nil, fmt.Sprintf("test-vpc-batch-v1-%v", i), db.GetStrPtr(fmt.Sprintf("test-vpc-desc-batch-1-%v", i)), tn.Org, ip.ID, tn.ID, st.ID, db.GetStrPtr(VpcEthernetVirtualizer), db.GetUUIDPtr(uuid.New()), map[string]string{fmt.Sprintf("test-vpc-batch-key1-%v", i): fmt.Sprintf("test-vpc-batch-value1-%v", i)}, db.GetStrPtr(VpcStatusReady), tn.CreatedBy, &networkSecurityGroup.ID)
		} else {
			vpc = testBuildVpc(t, dbSession, nil, fmt.Sprintf("test-vpc-batch-v2-%v", i), db.GetStrPtr(fmt.Sprintf("test-vpc-desc-batch-2-%v", i)), tn.Org, ip.ID, tn.ID, st.ID, db.GetStrPtr(VpcFNN), db.GetUUIDPtr(uuid.New()), map[string]string{fmt.Sprintf("test-vpc-batch-key2-%v", i): fmt.Sprintf("test-vpc-batch-value2-%v", i)}, db.GetStrPtr(VpcStatusDeleting), tn.CreatedBy, &networkSecurityGroup.ID)
		}
...
func TestVpcSQLDAO_UpdateFromParams(t *testing.T) {
	...
	vpc := testBuildVpc(t, dbSession, nil, "test-vpc", nil, tn.Org, ip.ID, tn.ID, st.ID, db.GetStrPtr(VpcEthernetVirtualizer), nil, nil, nil, tnu.ID, &networkSecurityGroup.ID)
...
func TestVpcSQLDAO_ClearFromParams(t *testing.T) {
	...
	vpc := testBuildVpc(t, dbSession, nil, "test-vpc", db.GetStrPtr("Test Description"), tn.Org, ip.ID, tn.ID, st.ID, db.GetStrPtr(VpcEthernetVirtualizer), db.GetUUIDPtr(uuid.New()), nil, db.GetStrPtr(VpcStatusReady), tnu.ID, &networkSecurityGroup.ID)
