...
import (
	...
	apiHandler "github.com/nvidia/carbide-rest-api/carbide-rest-api/pkg/api/handler"
...
func NewV2APIRoutes(dbSession *cdb.Session, tc tClient.Client, tnc tClient.NamespaceClient, scp *sc.ClientPool, cfg *config.Config) []Route {
	apiRoutes := []Route{
		...
		{
			Path:    "/org/:orgName/carbide/nvlinklogical-partition",
			Method:  http.MethodPost,
		...
		{
			Path:    "/org/:orgName/carbide/nvlinklogical-partition/:id",
			Method:  http.MethodGet,
