...
import (
	...
	csm "github.com/nvidia/carbide-rest-api/carbide-rest-site-manager/pkg/sitemgr"
...
func (gsh GetSiteHandler) Handle(c echo.Context) error {
	...
	isProviderRequest, orgProvider, orgTenant, apiErr := common.GetIsProviderRequest(ctx, logger, gsh.dbSession, org, dbUser,
	...
	if isProviderRequest {
		...
		var serr error
		ts, serr = tsDAO.GetByTenantIDAndSiteID(ctx, nil, orgTenant.ID, stID, nil)
		if serr != nil {
...
func (gash GetAllSiteHandler) Handle(c echo.Context) error {
	...
	ipStrID := c.QueryParam("infrastructureProviderId")
	tnStrID := c.QueryParam("tenantId")

	if ipStrID == "" && tnStrID == "" {
		return cerr.NewAPIErrorResponse(c, http.StatusBadRequest, "Must specify either Infrastructure Provider ID or Tenant ID in query", nil)
	} else if ipStrID != "" && tnStrID != "" {
		return cerr.NewAPIErrorResponse(c, http.StatusBadRequest, "Cannot specify both Infrastructure Provider ID and Tenant ID in query", nil)
	}
	...
	if ipStrID != "" {
		...
		ipID, serr := uuid.Parse(ipStrID)
		if serr != nil {
			return cerr.NewAPIErrorResponse(c, http.StatusBadRequest, "Invalid Infrastructure Provider ID in query", nil)
		}

		// Check that Infrastructure Provider exists
		ipDAO := cdbm.NewInfrastructureProviderDAO(gash.dbSession)

		ip, serr := ipDAO.GetByID(ctx, nil, ipID, nil)
		if serr != nil {
			logger.Warn().Err(serr).Msg("error retrieving Infrastructure Provider from DB by ID")
			return cerr.NewAPIErrorResponse(c, http.StatusNotFound, "Could not retrieve Infrastructure Provider with ID in query", nil)
		}

		if ip.Org != org {
			return cerr.NewAPIErrorResponse(c, http.StatusBadRequest, "Infrastructure Provider specified in query does not belong to current org", nil)
		}
		filter.InfrastructureProviderID = &ipID
		...
		sts, total, serr = stDAO.GetAll(ctx, nil, filter, paginator.PageInput{Offset: pageRequest.Offset, Limit: pageRequest.Limit, OrderBy: pageRequest.OrderBy}, qIncludeRelations)
		if serr != nil {
		...
		tnID, serr := uuid.Parse(tnStrID)
		if serr != nil {
			return cerr.NewAPIErrorResponse(c, http.StatusBadRequest, "Invalid Tenant ID in query", nil)
		}

		// Check that Tenant exists for org
		tnDAO := cdbm.NewTenantDAO(gash.dbSession)

		tn, serr := tnDAO.GetByID(ctx, nil, tnID, nil)
		if serr != nil {
			logger.Warn().Err(serr).Msg("error retrieving Tenant from DB by ID")
			return cerr.NewAPIErrorResponse(c, http.StatusNotFound, "Could not retrieve Tenant with ID in query", nil)
		}

		if tn.Org != org {
			return cerr.NewAPIErrorResponse(c, http.StatusBadRequest, "Tenant specified in query does not belong to current org", nil)
		}
		...
		tss, _, serr := tsDAO.GetAll(
			ctx,
			nil,
			cdbm.TenantSiteFilterInput{
				TenantIDs: []uuid.UUID{tnID},
			},
	...
	if includeMachineStats {
		...
		machineStats, err = common.GetSiteMachineCountStats(ctx, nil, gash.dbSession, logger, filter.InfrastructureProviderID, nil)
		if err != nil {
	...
	sdEntityIDs := []string{}
	for _, st := range sts {
		sdEntityIDs = append(sdEntityIDs, st.ID.String())
	}
	...
	asts := []model.APISite{}
	for _, st := range sts {
		ast := model.NewAPISite(st, ssdMap[st.ID.String()], tsMap[st.ID])

		// Attach machine stats for the site if they exist.
		ast.MachineStats = machineStats[st.ID]
	...
	return c.JSON(http.StatusOK, asts)
}
