...
func (csh CreateVpcPrefixHandler) Handle(c echo.Context) error {
	...
	vpc, err := common.GetVpcFromIDString(ctx, nil, apiRequest.VpcID, csh.dbSession)
	if err != nil {
	...
	createVpcPrefixRequest := &cwssaws.VpcPrefixCreationRequest{
		Id:     &cwssaws.UUID{Value: vpcPrefix.ID.String()},
		Name:   vpcPrefix.Name,
		VpcId:  &cwssaws.UUID{Value: common.GetSiteVpcID(vpc).String()},
...
func (gash GetAllVpcPrefixHandler) Handle(c echo.Context) error {
	...
	if qVpcID != "" {
		vpc, err := common.GetVpcFromIDString(ctx, nil, qVpcID, gash.dbSession)
		if err != nil {
			logger.Warn().Err(err).Msg("error getting vpc in request")
...
func (dsh DeleteVpcPrefixHandler) Handle(c echo.Context) error {
	...
	*/
	...
	deleteVpcPrefixRequest := &cwssaws.VpcDeletionRequest{
		Id: &cwssaws.UUID{Value: vpcPrefix.ID.String()},
	}
