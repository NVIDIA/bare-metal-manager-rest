...
func (cdesh CreateDpuExtensionServiceHandler) Handle(c echo.Context) error {
	...
	if err != nil {
		logger.Error().Err(err).Msg("failed to execute CreateDpuExtensionService workflow on Site")
		return cerr.NewAPIErrorResponse(c, http.StatusInternalServerError, "Failed to initiate DPU Extension Service creation workflow", nil)
	}
	...
	if err != nil {
		...
		if errors.As(err, &timeoutErr) {
			logger.Error().Err(err).Msg("Temporal workflow execution timed out on Site")
			return cerr.NewAPIErrorResponse(c, http.StatusInternalServerError, fmt.Sprintf("Timed out executing DPU Extension Service creation workflow on Site: %v", err), nil)
		}

		logger.Error().Err(err).Msg("error executing Temporal workflow on Site")
		return cerr.NewAPIErrorResponse(c, http.StatusInternalServerError, fmt.Sprintf("Failed to execute DPU Extension Service creation workflow on Site: %v", err), nil)
	}
	...
	if controllerDpuExtensionService != nil && controllerDpuExtensionService.LatestVersionInfo != nil {
		...
		created, err := time.Parse(time.RFC3339, controllerDpuExtensionService.LatestVersionInfo.Created)
		if err != nil {
...
func (udesh UpdateDpuExtensionServiceHandler) Handle(c echo.Context) error {
	...
	if err != nil {
		logger.Error().Err(err).Msg("failed to execute UpdateDpuExtensionService workflow on Site")
		return cerr.NewAPIErrorResponse(c, http.StatusInternalServerError, "Failed to initiate DPU Extension Service update workflow", nil)
	}
	...
	if err != nil {
		...
		logger.Error().Err(err).Msg("error executing Temporal workflow on Site")
		return cerr.NewAPIErrorResponse(c, http.StatusInternalServerError, fmt.Sprintf("Failed to execute DPU Extension Service update workflow on Site: %v", err), nil)
	}
...
func (gdesvh GetDpuExtensionServiceVersionHandler) Handle(c echo.Context) error {
	...
	if err != nil {
		logger.Error().Err(err).Msg("failed to execute sync Temporal workflow on Site")
		return cerr.NewAPIErrorResponse(c, http.StatusInternalServerError, fmt.Sprintf("Failed to execute sync workflow: %s on Site: %v", "GetDpuExtensionServiceVersionsInfo", err), nil)
	}
	...
	if err != nil {
		...
		if errors.As(err, &timeoutErr) {
			logger.Error().Err(err).Msg("Temporal workflow execution timed out on Site")
			return cerr.NewAPIErrorResponse(c, http.StatusInternalServerError, fmt.Sprintf("Timed out executing sync workflow: %s on Site: %v", "GetDpuExtensionServiceVersionsInfo", err), nil)
		}
	...
	if versionInfo.Created != "" {
		created, err = time.Parse(time.RFC3339, versionInfo.Created)
		if err != nil {
			created = time.Time{}
		}
	}
...
func (ddesvh DeleteDpuExtensionServiceVersionHandler) Handle(c echo.Context) error {
	...
	// Trigger Site workflow
	apiErr := common.ExecuteSyncWorkflow(ctx, logger, stc, "DeleteDpuExtensionServiceVersion", workflowOptions, deleteDpuExtensionServiceVersionRequest)
	if apiErr != nil {
		return cerr.NewAPIErrorResponse(c, apiErr.Code, apiErr.Message, apiErr.Data)
	}
