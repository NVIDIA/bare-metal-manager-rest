...
func (cemh CreateExpectedMachineHandler) Handle(c echo.Context) error {
	...
	infrastructureProvider, apiError := common.IsProvider(ctx, logger, cemh.dbSession, org, dbUser, false)
	if apiError != nil {
	...
	if verr != nil {
		...
		return cerr.NewAPIErrorResponse(c, http.StatusBadRequest, "Error validating Expected Machine creation request data", verr)
	}
...
func (gaemh GetAllExpectedMachineHandler) Handle(c echo.Context) error {
	...
	// ensure our user is a provider for the org
	infrastructureProvider, apiError := common.IsProvider(ctx, logger, gaemh.dbSession, org, dbUser, true)
	if apiError != nil {
		return cerr.NewAPIErrorResponse(c, apiError.Code, apiError.Message, apiError.Data)
	}
...
func (gemh GetExpectedMachineHandler) Handle(c echo.Context) error {
	...
	// ensure our user is a provider for the org
	infrastructureProvider, apiError := common.IsProvider(ctx, logger, gemh.dbSession, org, dbUser, true)
	if apiError != nil {
		return cerr.NewAPIErrorResponse(c, apiError.Code, apiError.Message, apiError.Data)
	}
...
func (uemh UpdateExpectedMachineHandler) Handle(c echo.Context) error {
	...
	// Ensure our user is a provider for the org
	infrastructureProvider, apiError := common.IsProvider(ctx, logger, uemh.dbSession, org, dbUser, false)
	if apiError != nil {
		return cerr.NewAPIErrorResponse(c, apiError.Code, apiError.Message, apiError.Data)
	}
...
func (demh DeleteExpectedMachineHandler) Handle(c echo.Context) error {
	...
	// Ensure our user is a provider for the org
	infrastructureProvider, apiError := common.IsProvider(ctx, logger, demh.dbSession, org, dbUser, false)
	if apiError != nil {
		return cerr.NewAPIErrorResponse(c, apiError.Code, apiError.Message, apiError.Data)
	}
	...
	if err != nil {
		logger.Error().Err(err).Msg("unable to start transaction")
		return cerr.NewAPIErrorResponse(c, http.StatusInternalServerError, "Failed to delete ExpectedMachine", nil)
	}
	// this variable is used in cleanup actions to indicate if this transaction committed
	txCommitted := false
	defer common.RollbackTx(ctx, tx, &txCommitted)
