...
func TestIPBlockHandler_Get(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:           "error when tenant in org doesnt match tenant in ipblock",
			reqOrgName:     tnOrg2,
			user:           tnu,
			ipbID:          ipb3.ID.String(),
			queryTenantID:  cdb.GetStrPtr(tn2.ID.String()),
			expectedErr:    true,
			expectedStatus: http.StatusNotFound,
		},
		{
		...
		},
		{
			...
			user:                     tnu,
			ipbID:                    ipb3.ID.String(),
			queryTenantID:            cdb.GetStrPtr(tn.ID.String()),
			expectedErr:              false,
			expectedStatus:           http.StatusOK,
		...
		},
		{
			...
			queryInfrastructureProviderID: cdb.GetStrPtr(ip.ID.String()),
			queryTenantID:                 cdb.GetStrPtr(tn3.ID.String()),
			expectedErr:                   false,
			expectedStatus:                http.StatusOK,
		...
		{
			name:                              "success when both infrastructure id and tenant id are specified with include relation",
			reqOrgName:                        ipOrg1,
			user:                              ipu,
			ipbID:                             ipb5.ID.String(),
			queryInfrastructureProviderID:     cdb.GetStrPtr(ip.ID.String()),
		...
		{
			name:                          "success when ipam usage requested",
			reqOrgName:                    ipOrg1,
			user:                          ipu,
			ipbID:                         ipb1.ID.String(),
			queryInfrastructureProviderID: cdb.GetStrPtr(ip.ID.String()),
		...
		{
			name:                          "success when ipam usage requested when full grant enabled",
			reqOrgName:                    ipOrg2,
			user:                          ipu,
			ipbID:                         ipb2.ID.String(),
		...
		{
			name:                     "success ipam usage requested when when tenant id is specified",
			reqOrgName:               tnOrg1,
			user:                     tnu,
			ipbID:                    ipb3.ID.String(),
			queryTenantID:            cdb.GetStrPtr(tn.ID.String()),
...
func TestIPBlockHandler_GetAll(t *testing.T) {
	...
	ip3 := testIPBlockBuildInfrastructureProvider(t, dbSession, "TestIp3", ipOrg3, ipu)
	assert.NotNil(t, ip3)
	ip4 := testIPBlockBuildInfrastructureProvider(t, dbSession, "TestIp4", ipOrg4, ipu)
	assert.NotNil(t, ip4)
	...
	site3 := testIPBlockBuildSite(t, dbSession, ip3, "testSite3", cdbm.SiteStatusRegistered, true, ipu)
	assert.NotNil(t, site3)
	site4 := testIPBlockBuildSite(t, dbSession, ip4, "testSite4", cdbm.SiteStatusRegistered, true, ipu)
	assert.NotNil(t, site4)

	tn := testIPBlockBuildTenant(t, dbSession, "testTenant", tnOrg1, tnu)
	assert.NotNil(t, tn)
	tn2 := testIPBlockBuildTenant(t, dbSession, "testTenant2", tnOrg1, tnu)
	assert.NotNil(t, tn2)
	...
	tests := []struct {
		...
		{
			name:           "success when tenant id and site id with no IP block is specified",
			reqOrgName:     tnOrg1,
			user:           tnu,
			queryTenantID:  cdb.GetStrPtr(tn.ID.String()),
		...
		{
			name:           "success when IPBlockStatusPending status query search specified",
			reqOrgName:     tnOrg1,
			user:           tnu,
			queryTenantID:  cdb.GetStrPtr(tn.ID.String()),
		...
		{
			name:           "success when BadStatus status query search specified",
			reqOrgName:     tnOrg1,
			user:           tnu,
			queryTenantID:  cdb.GetStrPtr(tn.ID.String()),
		...
		{
			name:                          "success ipam usage when infrastructure provider id is specified with full grant",
			reqOrgName:                    ipOrg4,
			user:                          ipu,
			queryInfrastructureProviderID: cdb.GetStrPtr(ip4.ID.String()),
			expectedErr:                   false,
			expectedStatus:                http.StatusOK,
			expectedCnt:                   2,
			expectedIpam:                  true,
