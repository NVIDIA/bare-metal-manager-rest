...
import (
	...
	"github.com/NVIDIA/carbide-rest-api/carbide-rest-workflow/pkg/queue"
...
func (cih CreateInstanceHandler) buildInstanceCreateRequestOsConfig(c echo.Context, logger *zerolog.Logger, apiRequest *model.APIInstanceCreateRequest, siteID uuid.UUID) (*cwssaws.OperatingSystem, *uuid.UUID, *cerr.APIError) {
	...
	// Options below should all have been set set by the
...
func (cih CreateInstanceHandler) Handle(c echo.Context) error {
	...
	vpc, err := common.GetVpcFromIDString(ctx, nil, apiRequest.VpcID, cih.dbSession)
	if err != nil {
	...
	ins, tot, err := inDAO.GetAll(ctx, nil, cdbm.InstanceFilterInput{Name: &apiRequest.Name, TenantIDs: []uuid.UUID{tenant.ID}, SiteIDs: []uuid.UUID{vpc.SiteID}}, cdbp.PageInput{}, nil)
	if err != nil {
	...
	if apiRequest.InstanceTypeID != nil {
		...
		totalConstaintValue := 0
		for _, alcs := range alconstraints {
			totalConstaintValue += alcs.ConstraintValue
		}

		// Allocation constraints
		if len(alconstraints) > 0 && insTotal >= totalConstaintValue {
			return cerr.NewAPIErrorResponse(c, http.StatusForbidden,
	...
	for _, dbifc := range dbifcs {
		...
		if dbifc.SubnetID != nil {
			interfaceConfig.NetworkSegmentId = &cwssaws.UUID{Value: subnets[*dbifc.SubnetID].ControllerNetworkSegmentID.String()}
			interfaceConfig.NetworkDetails = &cwssaws.InstanceInterfaceConfig_SegmentId{
				SegmentId: &cwssaws.UUID{Value: subnets[*dbifc.SubnetID].ControllerNetworkSegmentID.String()},
			}
		}
		...
		if dbifc.VpcPrefixID != nil {
			interfaceConfig.NetworkDetails = &cwssaws.InstanceInterfaceConfig_VpcPrefixId{
				VpcPrefixId: &cwssaws.UUID{Value: dbifc.VpcPrefixID.String()},
			}
		}
	...
	for _, ibifc := range dbibic {
		...
		ibInterfaceConfig := &cwssaws.InstanceIBInterfaceConfig{
			...
			IbPartitionId:  &cwssaws.UUID{Value: ifc.InfiniBandPartitionID.String()},
		}
	...
	createInstanceRequest := &cwssaws.InstanceAllocationRequest{
		InstanceId: &cwssaws.UUID{Value: common.GetSiteInstanceID(instance).String()},
	...
	// Create response
	apiInstance := model.NewAPIInstance(instance, site, ifcs, ibifcs, rdbskg, []cdbm.StatusDetail{*ssd})
...
func (uih UpdateInstanceHandler) handleReboot(c echo.Context, logger *zerolog.Logger, apiRequest *model.APIInstanceUpdateRequest, instance *cdbm.Instance) error {
	...
	// Create response
	apiInstance := model.NewAPIInstance(ui, instance.Site, retifc, nil, dbskgs, ssds)
...
func (uih UpdateInstanceHandler) Handle(c echo.Context) error {
	...
	logger = logger.With().Str("Tenant ID", instance.Tenant.ID.String()).Logger()
	...
	if instance.Site == nil {
		...
		return cerr.NewAPIErrorResponse(c, http.StatusBadRequest, "Failed to retrieve Site details for Instance", nil)
	}

	if instance.Site.Status != cdbm.SiteStatusRegistered {
		logger.Error().Msg("site not in registered state - cannot update Instance")
	...
	if apiRequest.Name != nil && *apiRequest.Name != instance.Name {
		ins, tot, serr := instanceDAO.GetAll(ctx, nil,
			cdbm.InstanceFilterInput{
				Name:      apiRequest.Name,
				TenantIDs: []uuid.UUID{instance.Tenant.ID},
				SiteIDs:   []uuid.UUID{instance.SiteID},
			},
			cdbp.PageInput{},
			nil,
		)
	...
	if apiRequest.NetworkSecurityGroupID != nil && *apiRequest.NetworkSecurityGroupID != "" {
		...
		if nsg.SiteID != instance.SiteID {
			logger.Error().Msg("NetworkSecurityGroup in request does not belong to Site")
			return cerr.NewAPIErrorResponse(c, http.StatusForbidden, "NetworkSecurityGroup with ID specified in request data does not belong to Site", nil)
		}

		if nsg.TenantID != instance.TenantID {
			logger.Error().Msg("NetworkSecurityGroup in request does not belong to Tenant")
			return cerr.NewAPIErrorResponse(c, http.StatusForbidden, "NetworkSecurityGroup with ID specified in request data does not belong to Tenant", nil)
		}
	...
	for _, ifc := range apiRequest.Interfaces {
		if ifc.SubnetID != nil {
			...
			if subnets[subnetID] == nil {
				...
				if subnet.TenantID != instance.TenantID {
					logger.Warn().Msg(fmt.Sprintf("Subnet: %v specified in request is not owned by Tenant", subnetID))
					return cerr.NewAPIErrorResponse(c, http.StatusBadRequest, fmt.Sprintf("Subnet: %v specified in request is not owned by Tenant", subnetID), nil)
				}
				...
				if subnet.VpcID != instance.VpcID {
					logger.Warn().Msg(fmt.Sprintf("Subnet: %v specified in request does not match with VPC", subnetID))
					return cerr.NewAPIErrorResponse(c, http.StatusBadRequest, fmt.Sprintf("Subnet: %v specified in request does not match with VPC", subnetID), nil)
				}

				if instance.Vpc.NetworkVirtualizationType != nil && *instance.Vpc.NetworkVirtualizationType != cdbm.VpcEthernetVirtualizer {
		...
		if ifc.VpcPrefixID != nil {
			...
			if vpcPrefixes[vpcPrefixID] == nil {
				...
				if vpcPrefix.TenantID != instance.TenantID {
					logger.Warn().Msg(fmt.Sprintf("VPC Prefix: %v specified in request is not owned by Tenant", vpcPrefixID))
					return cerr.NewAPIErrorResponse(c, http.StatusBadRequest, fmt.Sprintf("VPC Prefix: %v specified in request is not owned by Tenant", vpcPrefixID), nil)
				}
				...
				if vpcPrefix.VpcID != instance.VpcID {
					logger.Warn().Msg(fmt.Sprintf("VPC Prefix: %v specified in request does not match with VPC", vpcPrefixID))
					return cerr.NewAPIErrorResponse(c, http.StatusBadRequest, fmt.Sprintf("VPC Prefix: %v specified in request does not match with VPC", vpcPrefixID), nil)
				}

				if instance.Vpc.NetworkVirtualizationType == nil || *instance.Vpc.NetworkVirtualizationType != cdbm.VpcFNN {
	...
	for _, ibic := range apiRequest.InfiniBandInterfaces {
		...
		if ibp.SiteID != instance.SiteID {
			logger.Warn().Msg(fmt.Sprintf("InfiniBandPartition: %v specified in request does not match with Instance Site", ibpID))
			return cerr.NewAPIErrorResponse(c, http.StatusBadRequest, fmt.Sprintf("Partition: %v specified in request does not match with Instance Site", ibpID), nil)
		}

		if ibp.TenantID != instance.TenantID {
			logger.Warn().Msg(fmt.Sprintf("InfiniBandPartition: %v specified in request is not owned by Tenant", ibpID))
			return cerr.NewAPIErrorResponse(c, http.StatusBadRequest, fmt.Sprintf("Partition: %v specified in request is not owned by Tenant", ibpID), nil)
		}
	...
	osConfig, osID, oserr := uih.buildInstanceUpdateRequestOsConfig(c, &logger, &apiRequest, instance, instance.SiteID)
	if oserr != nil {
	...
	siteDAO := cdbm.NewSiteDAO(uih.dbSession)
	site, err := siteDAO.GetByID(ctx, tx, ui.SiteID, nil, false)
	if err != nil {
	...
	curifcs, _, err := ifcDAO.GetAll(ctx, tx, cdbm.InterfaceFilterInput{InstanceIDs: []uuid.UUID{instance.ID}}, cdbp.PageInput{OrderBy: &cdbp.OrderBy{Field: cdbm.InterfaceOrderByCreated, Order: cdbp.OrderAscending}}, []string{cdbm.SubnetRelationName, cdbm.VpcPrefixRelationName})
	if err != nil {
	...
	var oldUpdateddbifcs []cdbm.Interface
	if apiRequest.Interfaces != nil && len(apiRequest.Interfaces) > 0 {
		for _, dbifc := range dbifcs {
			input := cdbm.InterfaceCreateInput{
				InstanceID:        instance.ID,
				SubnetID:          dbifc.SubnetID,
		...
		for _, dbifc := range curifcs {
			dbifc.Status = cdbm.InterfaceStatusDeleting
			udbifc, err := ifcDAO.Update(ctx, tx, cdbm.InterfaceUpdateInput{InterfaceID: dbifc.ID, Status: cdb.GetStrPtr(cdbm.InterfaceStatusDeleting)})
			if err != nil {
			...
			// Add the old updated Interface to the list of old Interfaces that were updated
			oldUpdateddbifcs = append(oldUpdateddbifcs, *udbifc)
		}
	} else {
		newdbifcs = curifcs
	}
	...
	var dbibifcs []cdbm.InfiniBandInterface
	...
	curdbibifcs, _, err := ibiDAO.GetAll(ctx, tx, cdbm.InfiniBandInterfaceFilterInput{InstanceIDs: []uuid.UUID{instanceID}}, cdbp.PageInput{OrderBy: &cdbp.OrderBy{Field: cdbm.InfiniBandInterfaceOrderByCreated, Order: cdbp.OrderAscending}}, nil)
	if err != nil {
	...
	if apiRequest.InfiniBandInterfaces != nil {
		for _, apiibifc := range apiRequest.InfiniBandInterfaces {
			...
			dbibifcs = append(dbibifcs, *dbibifc)
		}

		// Update status of existing InfiniBand Interfaces to Deleting
		for _, dbibifc := range curdbibifcs {
			dbibifc.Status = cdbm.InfiniBandInterfaceStatusDeleting
			_, err = ibiDAO.Update(ctx, tx, cdbm.InfiniBandInterfaceUpdateInput{InfiniBandInterfaceID: dbibifc.ID, Status: cdb.GetStrPtr(cdbm.InfiniBandInterfaceStatusDeleting)})
			if err != nil {
	...
	for i, ifc := range newdbifcs {
		...
		if ifc.SubnetID != nil {
			interfaceConfig.NetworkSegmentId = &cwssaws.UUID{Value: ifc.SubnetID.String()}
			interfaceConfig.NetworkDetails = &cwssaws.InstanceInterfaceConfig_SegmentId{
				SegmentId: &cwssaws.UUID{Value: ifc.SubnetID.String()},
			}
		}

		if ifc.VpcPrefixID != nil {
			interfaceConfig.NetworkDetails = &cwssaws.InstanceInterfaceConfig_VpcPrefixId{
				VpcPrefixId: &cwssaws.UUID{Value: ifc.VpcPrefixID.String()},
			}
		}
	...
	for _, dbibifc := range dbibifcs {
		if dbibifc.Status == cdbm.InfiniBandInterfaceStatusDeleting {
			// NOTE: Don't send any InfiniBand Interfaces that are being deleted
			continue
		}

		ibInterfaceConfig := &cwssaws.InstanceIBInterfaceConfig{
			Device:         dbibifc.Device,
			Vendor:         dbibifc.Vendor,
			DeviceInstance: uint32(dbibifc.DeviceInstance),
			FunctionType:   cwssaws.InterfaceFunctionType_PHYSICAL_FUNCTION,
			IbPartitionId:  &cwssaws.UUID{Value: dbibifc.InfiniBandPartitionID.String()},
		}

		// NOTE: Not supported yet, but ensures future compatibility
		if !dbibifc.IsPhysical {
			ibInterfaceConfig.FunctionType = cwssaws.InterfaceFunctionType_VIRTUAL_FUNCTION

			if dbibifc.VirtualFunctionID != nil {
				vfID := uint32(*dbibifc.VirtualFunctionID)
				ibInterfaceConfig.VirtualFunctionId = &vfID
			}
		}
	...
	updateInstanceRequest := &cwssaws.InstanceConfigUpdateRequest{
		...
		Config: &cwssaws.InstanceConfig{
			...
			Tenant: &cwssaws.TenantConfig{
				TenantOrganizationId: instance.Tenant.Org,
				TenantKeysetIds:      instanceSshKeyGroupIds,
			},
			Os: osConfig,
			Network: &cwssaws.InstanceNetworkConfig{
	...
	// If old Interfaces were updated, add them to the response
	if oldUpdateddbifcs != nil && len(oldUpdateddbifcs) > 0 {
		// Add the old Interfaces to the response
		newdbifcs = append(newdbifcs, oldUpdateddbifcs...)
	}

	// Create response
	apiInstance := model.NewAPIInstance(ui, site, newdbifcs, dbibifcs, dbskgs, ssds)
...
func (gih GetInstanceHandler) Handle(c echo.Context) error {
	...
	retifc, _, err := ifcDAO.GetAll(ctx, nil, cdbm.InterfaceFilterInput{InstanceIDs: []uuid.UUID{instance.ID}}, cdbp.PageInput{}, []string{cdbm.SubnetRelationName, cdbm.VpcPrefixRelationName})
	if err != nil {
	...
	retibifc, _, err := ibifcDAO.GetAll(
		ctx,
		nil,
	...
	// Create response
	ins := model.NewAPIInstance(instance, site, retifc, retibifc, dbskgs, ssds)
...
func (gaih GetAllInstanceHandler) Handle(c echo.Context) error {
	...
	if infrastructureProviderIDStr != "" {
		...
		filter.InfrastructureProviderID = &parsedID
	}
	...
	if vpcIDStrs := qParams["vpcId"]; len(vpcIDStrs) != 0 {
		...
		for _, vpcIDStr := range vpcIDStrs {
			...
			vpc, verr := common.GetVpcFromIDString(ctx, nil, vpcIDStr, gaih.dbSession)
			if verr != nil {
	...
	for _, ins := range dbInstances {
		...
		// Create response
		dbInstance := ins
		apiInstance := model.NewAPIInstance(&dbInstance, sitesByID[dbInstance.SiteID], ifcMap[dbInstance.ID], ibifcMap[dbInstance.ID], skgiasMap[dbInstance.ID], ssdMap[ins.ID.String()])
...
func (dih DeleteInstanceHandler) Handle(c echo.Context) error {
	...
	releaseInstanceRequest := &cwssaws.InstanceReleaseRequest{
		Id: &cwssaws.UUID{Value: common.GetSiteInstanceID(instance).String()},
	}
