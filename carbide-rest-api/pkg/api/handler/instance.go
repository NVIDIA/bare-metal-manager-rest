...
func (cih CreateInstanceHandler) Handle(c echo.Context) error {
	...
	// Validate NVLink interfaces if interfaces in request specified
...
func (uih UpdateInstanceHandler) Handle(c echo.Context) error {
	...
	if len(apiRequest.Interfaces) > 0 {
		...
		for _, dbifc := range existingIfcs {
			...
			existingIfcs = append(existingIfcs, *udbifc)
		}
	} else {
	...
	if apiRequest.InfiniBandInterfaces != nil {
		...
		for _, dbibifc := range existingIbIfcs {
	...
	if apiRequest.NVLinkInterfaces != nil {
		...
		for _, dbnvlifc := range existingNvlIfcs {
			dbnvlifc.Status = cdbm.NVLinkInterfaceStatusDeleting
			_, err := nvlIfcDAO.Update(ctx, tx, cdbm.NVLinkInterfaceUpdateInput{NVLinkInterfaceID: dbnvlifc.ID, Status: cdb.GetStrPtr(cdbm.NVLinkInterfaceStatusDeleting)})
			if err != nil {
