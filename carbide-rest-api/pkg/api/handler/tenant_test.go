...
func TestGetCurrentTenantStatsHandler_Handle(t *testing.T) {
	...
	vpc := testVPCBuildVPC(t, dbSession, "test-vpc", ip, tn1, st, cdb.GetStrPtr(cdbm.VpcEthernetVirtualizer), map[string]string{"zone": "east1"}, cdbm.VpcStatusReady, tnu1)
	assert.NotNil(t, vpc)
	...
	vpc2 := testInstanceBuildVPC(t, dbSession, "test-vpc-2", ip, tn1, st, nil, nil, cdb.GetStrPtr(cdbm.VpcEthernetVirtualizer), cdbm.VpcStatusPending, tnu1)
