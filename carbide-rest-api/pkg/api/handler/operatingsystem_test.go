...
func TestOperatingSystemHandler_Create(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:                          "success case when imageURL specified in the request",
			reqOrgName:                    tnOrg2,
			reqBody:                       string(validImageURLBody),
			reqBodyModel:                  &osImageURLObj3,
			user:                          tnu,
			expectedImageURL:              true,
			expectedErr:                   false,
			expectedStatus:                http.StatusCreated,
			expectedOperatingSystemStatus: cdbm.OperatingSystemSiteAssociationStatusSyncing,
			expectedStatusHistoryCount:    1,
			verifyChildSpanner:            true,
		},
		{
			name:           "error case when imageURL specified in the request, context dealine timeout",
			reqOrgName:     tnOrg2,
