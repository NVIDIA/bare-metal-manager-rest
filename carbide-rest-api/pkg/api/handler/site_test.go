...
import (
	...
	csmtypes "github.com/nvidia/carbide-rest-api/carbide-rest-site-manager/pkg/types"
...
func TestGetSiteHandler_Handle(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "test Site retrieval failure when user has both Provider/Tenant role but no query param specified",
			fields: fields{
				dbSession: dbSession,
				tc:        &tmocks.Client{},
				cfg:       cfg,
		...
		{
			name: "test Site retrieval success when user has both Provider/Tenant role, Provider query param specified",
			fields: fields{
				dbSession: dbSession,
				tc:        &tmocks.Client{},
				cfg:       cfg,
...
func TestGetAllSiteHandler_Handle(t *testing.T) {
	...
	ipu2 := testSiteBuildUser(t, dbSession, "test1234", ipOrg, ipRoles)
	ip2 := testSiteBuildInfrastructureProvider(t, dbSession, "Test Infrastructure Provider 2", ipOrg, ipu2)
	...
	tests := []struct {
	...
	}{
		{
			...
			args: args{
				org: ipOrg,
				query: url.Values{
					"infrastructureProviderId": []string{ip1.ID.String()},
				},
				user: ipu1,
			},
		...
		},
		{
			...
			args: args{
				org: ipOrg,
				query: url.Values{
					"infrastructureProviderId": []string{ip2.ID.String()},
