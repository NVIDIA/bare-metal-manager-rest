...
const (
	...
	errMsgInstanceSubnetsDeprecated          = "'instanceSubnets' field is deprecated. Please use 'interfaces'"
...
type APIInstanceCreateRequest struct {
	...
	// InstanceSubnets is the list of Subnets association for the Instance
	// NOTE: This field has been deprecated
	InstanceSubnets []APIInterfaceCreateOrUpdateRequest `json:"instanceSubnets"`
...
func (icr APIInstanceCreateRequest) Validate() error {
	err := validation.ValidateStruct(&icr,
		...
		validation.Field(&icr.OperatingSystemID,
			validationis.UUID.Error(validationErrorInvalidUUID)),
		validation.Field(&icr.InstanceSubnets,
			validation.Nil.Error(errMsgInstanceSubnetsDeprecated)),
...
func (iur *APIInstanceUpdateRequest) IsInterfaceUpdateRequest() bool {
	return iur.Interfaces != nil || iur.InfiniBandInterfaces != nil
}
...
type APIInstance struct {
	...
	SSHKeyGroupIDs []string `json:"sshKeyGroupIds"`
	// Deprecated: This attribute was mistakenly added without proper camelCase convention
	SSHKeyGroupsLegacy *SSHKeyGroupsSummaryDeprecated `json:"sshkeygroups,omitempty"`
...
func NewAPIInstance(dbinst *cdbm.Instance, dbSite *cdbm.Site, dbiss []cdbm.Interface, dbibis []cdbm.InfiniBandInterface, dbskgs []cdbm.SSHKeyGroup, dbsds []cdbm.StatusDetail) *APIInstance {
	...
	if time.Now().Before(sshKeyGroupsLegacyDeprecatedTime) {
		apiInstance.SSHKeyGroupsLegacy = &SSHKeyGroupsSummaryDeprecated{
			SSHKeyGroups: []APISSHKeyGroupSummary{},
		}
	}
	...
	for _, dbskg := range dbskgs {
		...
		if time.Now().Before(sshKeyGroupsLegacyDeprecatedTime) {
			apiInstance.SSHKeyGroupsLegacy.SSHKeyGroups = append(apiInstance.SSHKeyGroupsLegacy.SSHKeyGroups, *NewAPISSHKeyGroupSummary(&skg))
		}
	}
