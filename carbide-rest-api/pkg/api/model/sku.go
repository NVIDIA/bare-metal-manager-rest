...
type APISkuTpm struct {
	...
	Vendor string `json:"vendor"`
	// Model describes the model of the TPM
	Model string `json:"model"`
	// Count describes the number of TPMs present
	Count uint32 `json:"count"`
}
...
func NewAPISkuComponents(protoComponents *cwssaws.SkuComponents) *APISkuComponents {
	...
	// Map Tpm components
	if len(protoComponents.Tpm) > 0 {
		apiComponents.Tpm = []APISkuTpm{}
		for _, tpm := range protoComponents.Tpm {
			apiComponents.Tpm = append(apiComponents.Tpm, APISkuTpm{
				Vendor: tpm.Vendor,
				Model:  tpm.Model,
				Count:  tpm.Count,
			})
