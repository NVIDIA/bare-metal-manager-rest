...
func TestNewAPIInstance(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := NewAPIInstance(tt.args.dbic, tt.args.dbs, tt.args.dbis, tt.args.dbibi, tt.args.dbskg, tt.args.dbsds)
			...
			if time.Now().Before(sshKeyGroupsLegacyDeprecatedTime) {
				assert.NotNil(t, attrMap["sshkeygroups"])
				assert.Equal(t, len(tt.args.dbskg), len(attrMap["sshkeygroups"].([]interface{})))
			} else {
				_, exists := attrMap["sshkeygroups"]
				assert.False(t, exists, string(jsonResp))
			}
		})
	}
}
...
func TestAPIInstanceCreateRequest_Validate(t *testing.T) {
	type fields struct {
		...
		IpxeScript           *string
		UserData             *string
		InstanceSubnets      []APIInterfaceCreateOrUpdateRequest
	...
	tests := []struct {
		...
		{
			name: "test invalid Instance create request, neither InstanceSubnets nor Interfaces specified",
			fields: fields{
				Name:              "test-name",
				TenantID:          uuid.NewString(),
				AllocationID:      uuid.NewString(),
		...
		{
			name: "test invalid Instance create request, both InstanceSubnets and Interfaces specified",
			fields: fields{
				Name:              "test-name",
				TenantID:          uuid.NewString(),
				AllocationID:      uuid.NewString(),
		...
		{
			name: "test invalid Instance create request, InstanceSubnets specified",
			fields: fields{
				Name:              "test-name",
				TenantID:          uuid.NewString(),
				AllocationID:      uuid.NewString(),
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			icr := APIInstanceCreateRequest{
				...
				InstanceSubnets:   tt.fields.InstanceSubnets,
			...
			if tt.checkDefaultPhysical {
				if tt.fields.InstanceSubnets != nil {
					assert.True(t, tt.fields.InstanceSubnets[0].IsPhysical)
				}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.wantErr {
				...
				if itur.Interfaces != nil && len(itur.Interfaces) > 0 {
