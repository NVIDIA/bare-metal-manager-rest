...
func TestNewAPISkuWithFullComponents(t *testing.T) {
	...
	t.Run("complete GPU server with all component types", func(t *testing.T) {
		...
		// Validate TPM
		assert.Len(t, result.Components.Tpm, 1)
		assert.Equal(t, "Infineon", result.Components.Tpm[0].Vendor)
		assert.Equal(t, "SLB 9670", result.Components.Tpm[0].Model)
		assert.Equal(t, uint32(1), result.Components.Tpm[0].Count)
	})
...
func TestAPISkuComponentsWithSpecialValues(t *testing.T) {
	...
	t.Run("multiple components of each type", func(t *testing.T) {
		dbSku := &cdbm.SKU{
			...
			Components: &cdbm.SkuComponents{
				SkuComponents: &cwssaws.SkuComponents{
					...
					Tpm: []*cwssaws.SkuComponentTpm{
						{Vendor: "Infineon", Model: "TPM1", Count: 1},
						{Vendor: "Nuvoton", Model: "TPM2", Count: 1},
					},
				},
			},
			Created: createdTime,
		...
		assert.Len(t, result.Components.Tpm, 2)
		assert.Equal(t, "Infineon", result.Components.Tpm[0].Vendor)
	})
}
