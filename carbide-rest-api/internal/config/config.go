...
import (
	...
	cauth "github.com/NVIDIA/carbide-rest-api/carbide-rest-auth/pkg/config"
...
func NewConfig() *Config {
	...
	if c.GetKeycloakEnabled() {
		...
		} else if c.GetKeycloakClientSecret() != "" {
			log.Info().Msg("Keycloak is enabled, using client secret from environment variable")
		} else {
			log.Panic().Msg("Keycloak is enabled but neither client secret nor client secret path is set")
		}
	} else {
...
func (c *Config) Validate() {
	...
	if len(authConfigs) == 0 && !keycloakEnabled {
		log.Warn().Msg("No auth configurations specified and Keycloak is disabled - using development mode without authentication")
...
func (c *Config) GetOrInitJWTOriginConfig() *cauth.JWTOriginConfig {
	if c.JwtOriginConfig == nil {
		...
		authConfigs := c.GetAuthConfigs()
		for _, authConfig := range authConfigs {
			c.JwtOriginConfig.AddConfig(authConfig.Name, authConfig.Issuer, authConfig.URL, authConfig.Origin, authConfig.ServiceAccount, nil, nil)
		}
...
func (c *Config) GetSiteConfig() (*SiteConfig, error) {
	var err error
	if c.SiteConfig == nil {
		c.SiteConfig, err = NewSiteConfig(c.GetSitePhoneHomeUrl())
	}
	return c.SiteConfig, err
}
...
func (c *Config) WatchSecretFilePaths() {
	...
	if dbPassFileDir == "" && keycloakClientSecretFileDir == "" {
	...
	go func() {
		...
		if c.GetKeycloakEnabled() && keycloakClientSecretFileDir != "" {
