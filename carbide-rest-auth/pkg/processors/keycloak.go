...
func (h *KeycloakProcessor) ProcessToken(c echo.Context, tokenStr string, jwksConfig *config.JwksConfig, logger zerolog.Logger) (*cdbm.User, *util.APIError) {
	...
	// If oidc_id is not set, use sub as fallback (standard Keycloak doesn't provide oidc_id)
	if auxId == "" {
		auxId = sub
		logger.Info().Msg("Using subject claim as auxiliary ID (oidc_id not provided in token)")
	}
