...
import (
	...
	"github.com/NVIDIA/carbide-rest-api/carbide-rest-auth/pkg/config"
	commonConfig "github.com/NVIDIA/carbide-rest-api/carbide-rest-common/pkg/config"
	"github.com/NVIDIA/carbide-rest-api/carbide-rest-common/pkg/util"
...
func AuthProcessor(c echo.Context, joCfg *config.JWTOriginConfig) *util.APIError {
	...
	if c.Path() == EchoUnmatchedPath {
		logger.Warn().Msg("the requested path could not be found")
		return util.NewAPIError(http.StatusNotFound, "The requested path could not be found", nil)
	}
