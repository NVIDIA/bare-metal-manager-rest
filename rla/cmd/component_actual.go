...
func doGetActualComponents() {
	...
	componentType := parseComponentType(actualComponentType)
...
func outputActualTable(result *client.GetActualComponentsResult) {
	...
	for _, comp := range result.Components {
		lastSeen := ""
		if comp.LastSeen != nil {
			lastSeen = comp.LastSeen.AsTime().Format("2006-01-02 15:04")
		}

		compType := "compute"
		if comp.Type == pb.ComponentType_COMPONENT_TYPE_NVLSWITCH {
			compType = "nvlswitch"
		} else if comp.Type == pb.ComponentType_COMPONENT_TYPE_POWERSHELF {
			compType = "powershelf"
		}
