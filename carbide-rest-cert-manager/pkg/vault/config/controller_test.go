...
func TestVaultFSM(t *testing.T) {
	t.Skip("Skipping test that requires complex testdata setup with vault certificates and state")
	ts := &suite{}
	ts.setup(t)
	defer ts.teardown()
	
	// Create a temp directory with the expected secrets structure
	dir := t.TempDir()
	secretsDir := dir + "/secrets"
	
	// Create the directory structure that the controller expects
	// Based on constants: SecretTokenPath, SecretCACertPath, SecretCAPrivateKeyPath
	err := os.MkdirAll(secretsDir+"/token", 0755)
	require.NoError(t, err)
	err = os.MkdirAll(secretsDir+"/vault-root-ca-certificate", 0755)
	require.NoError(t, err)
	err = os.MkdirAll(secretsDir+"/vault-root-ca-private-key", 0755)
	require.NoError(t, err)

	vcc, err := NewController(fmt.Sprintf("http://%s", ts.l.Addr().String()), "https://test.example.com", secretsDir)
	require.NoError(t, err)
