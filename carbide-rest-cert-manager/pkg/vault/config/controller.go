...
func (v *VaultConfigController) run(ctx context.Context) {
	...
	if err := v.recoverState(); err != nil {
		...
		// Check if we're running in dev mode with a known root token
		v.tryDevModeToken(ctx)
	}
...
func (v *VaultConfigController) tryDevModeToken(ctx context.Context) {
	log := core.GetLogger(ctx)
	// Check if running in vault dev mode with a known root token
	devToken := os.Getenv("VAULT_DEV_ROOT_TOKEN_ID")
	if devToken != "" {
		log.Infof("Detected dev mode, using VAULT_DEV_ROOT_TOKEN_ID")
		// Create a mock InitResponse with the dev token
		initResp := &vault.InitResponse{
			RootToken: devToken,
			Keys:      []string{"dev-unseal-key"}, // Dev mode doesn't need unsealing
		}
		v.setToken(initResp)
	}
}
