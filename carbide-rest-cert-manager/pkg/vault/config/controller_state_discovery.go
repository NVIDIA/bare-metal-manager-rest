...
func (v *VaultConfigController) isPKICACertNotConfigured(ctx context.Context) (bool, error) {
	...
	if err != nil {
		// Check if it's a 400 error (no default issuer) which means PKI is enabled but not configured
		// This is expected and not an error condition for this state check
		errStr := err.Error()
		if strings.Contains(errStr, "Code: 400") || strings.Contains(errStr, "no default issuer") {
			log.Infof("PKI CA cert not configured (expected): %v", err)
			return true, nil
		}
		// For other errors (like auth issues), return the error
