...
func TestVaultConfigController_getSecret(t *testing.T) {
	secretsRootDir := filepath.Join(t.TempDir(), "secrets")
	testdataDir := filepath.Join(secretsRootDir, "token")
	err := os.MkdirAll(testdataDir, 0755)
	require.NoError(t, err)

	tokenContent := "774d5e5a7792a7a0f077291e272b4b0eb42b7ad2c18d5c7f4ad29ebb4c46be77"
	tokenFile := filepath.Join(testdataDir, "vault-token")
	err = os.WriteFile(tokenFile, []byte(tokenContent), 0644)
	require.NoError(t, err)

	vcc, err := NewController("http://localhost", "http://localhost", secretsRootDir)
	require.NoError(t, err)
