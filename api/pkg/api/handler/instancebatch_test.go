	...
	tests := []struct {
		...
		// DPU Extension Service failure tests
		{
			...
			args: args{
				...
				reqUser:  tnu1,
				respCode: http.StatusBadRequest,
				respMsg:  "Could not find DPU Extension Service with ID:",
			},
			wantErr: false,
		},
		{
			...
			args: args{
				...
				respMsg:  "does not belong to Site where Instance is being created",
			},
			wantErr: false,
		},
		// OperatingSystemID tests - covering buildBatchInstanceCreateRequestOsConfig OS branch
		{
			name: "test batch instance create API endpoint succeeds with valid OperatingSystemID",
			fields: fields{
				dbSession: dbSession,
				tc:        tc,
				scp:       scp,
		...
		{
			name: "test batch instance create API endpoint fails with OperatingSystem not in VPC site",
			fields: fields{
				dbSession: dbSession,
				tc:        tc,
				scp:       scp,
			...
			args: args{
				...
				reqUser:  tnu1,
				respCode: http.StatusBadRequest,
				respMsg:  "OperatingSystem specified in request is not in VPC site",
			},
			wantErr: false,
		},
		{
