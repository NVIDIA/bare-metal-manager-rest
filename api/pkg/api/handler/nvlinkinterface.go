...
// ~~~~~ GetAll NVLinkInterface By Instance Handler ~~~~~ //

// GetAllNVLinkInterfaceByInstanceHandler is the API Handler for retrieving all NVLinkInterfaces for an Instance
type GetAllNVLinkInterfaceByInstanceHandler struct {
...
// NewGetAllNVLinkInterfaceByInstanceHandler initializes and returns a new handler for retrieving all subnets for an Instance
func NewGetAllNVLinkInterfaceByInstanceHandler(dbSession *cdb.Session, tc temporalClient.Client, cfg *config.Config) GetAllNVLinkInterfaceByInstanceHandler {
	return GetAllNVLinkInterfaceByInstanceHandler{
		dbSession:  dbSession,
		tc:         tc,
		cfg:        cfg,
...
// @Param includeRelation query string false "Related entities to include in response e.g. 'Instance, NVLinkLogicalPartition'"
...
func (gaish GetAllNVLinkInterfaceByInstanceHandler) Handle(c echo.Context) error {
	// Get context
	...
	newctx, handlerSpan := gaish.tracerSpan.CreateChildInContext(ctx, "GetAllNVLinkInterfaceByInstanceHandler", logger)
	if handlerSpan != nil {
	...
	// Get the instance NVLink Interfaces record from the db
...
// ~~~~~ GetAll NVLinkInterface By NVLinkLogicalPartition Handler ~~~~~ //

// GetAllNVLinkInterfaceByNVLinkLogicalPartitionHandler is the API Handler for retrieving all NVLinkInterfaces for an NVLinkLogicalPartition
type GetAllNVLinkInterfaceByNVLinkLogicalPartitionHandler struct {
	dbSession  *cdb.Session
	tc         temporalClient.Client
	cfg        *config.Config
	tracerSpan *sutil.TracerSpan
...
// NewGetAllNVLinkInterfaceByNVLinkLogicalPartitionHandler initializes and returns a new handler for retrieving all NVLinkInterfaces for an NVLinkLogicalPartition
func NewGetAllNVLinkInterfaceByNVLinkLogicalPartitionHandler(dbSession *cdb.Session, tc temporalClient.Client, cfg *config.Config) GetAllNVLinkInterfaceByNVLinkLogicalPartitionHandler {
	return GetAllNVLinkInterfaceByNVLinkLogicalPartitionHandler{
		dbSession:  dbSession,
		tc:         tc,
		cfg:        cfg,
		tracerSpan: sutil.NewTracerSpan(),
...
// Handle godoc
// @Summary Retrieve all NVLinkInterfaces for an NVLinkLogicalPartition
// @Description Retrieve all NVLinkInterfaces for an NVLinkLogicalPartition
// @Tags NVLinkInterface
// @Accept json
// @Produce json
// @Security ApiKeyAuth
...
// @Param orderBy query string false "Order by field"
// @Success 200 {object} model.APINVLinkInterface
// @Router /v2/org/{org}/carbide/nvlink-logical-partition/{nvlinkLogicalPartitionId}/nvlink-interface [get]
func (gaish GetAllNVLinkInterfaceByNVLinkLogicalPartitionHandler) Handle(c echo.Context) error {
	// Get context
	ctx := c.Request().Context()
	...
	// Create a child span and set the attributes for current request
	newctx, handlerSpan := gaish.tracerSpan.CreateChildInContext(ctx, "GetAllNVLinkInterfaceByNVLinkLogicalPartitionHandler", logger)
	if handlerSpan != nil {
		// Set newly created span context as a current context
		ctx = newctx
	...
	nvlinkLogicalPartitionStrID := c.Param("nvlinkLogicalPartitionId")
	nvlinkLogicalPartitionID, err := uuid.Parse(nvlinkLogicalPartitionStrID)
	if err != nil {
	...
	filterInput := cdbm.NVLinkInterfaceFilterInput{
		NVLinkLogicalPartitionIDs: []uuid.UUID{nvlinkLogicalPartitionID},
	}
	...
	dbNVLinkInterfaces, total, err := nvlIfcDAO.GetAll(ctx, nil, filterInput, pageInput, qIncludeRelations)
	if err != nil {
		logger.Error().Err(err).Msg("error retrieving NVLink Interface by NVLink Logical Partition Details from DB")
		return cerr.NewAPIErrorResponse(c, http.StatusInternalServerError, "Failed to retrieve NVLink Interface by NVLink Logical Partition Details", nil)
	}
