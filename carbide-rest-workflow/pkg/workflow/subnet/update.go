...
func UpdateSubnetInventory(ctx workflow.Context, siteID string, subnetInventory *cwssaws.SubnetInventory) (err error) {
	...
	// Trigger workflow to record subnet lifecycle metrics
	serr := workflow.ExecuteActivity(ctx, subnetManager.RecordSubnetLifecycleMetrics, parsedSiteID, subnetLifecycleEvents).Get(ctx, nil)
	if serr != nil {
		logger.Warn().Err(serr).Msg("failed to execute activity: RecordSubnetLifecycleMetrics")
	}
