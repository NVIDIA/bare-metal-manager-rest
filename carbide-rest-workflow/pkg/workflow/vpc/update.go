...
func UpdateVpcInventory(ctx workflow.Context, siteID string, vpcInventory *cwssaws.VPCInventory) (err error) {
	...
	// Trigger workflow to record VPC lifecycle metrics
	serr := workflow.ExecuteActivity(ctx, vpcManager.RecordVpcLifecycleMetrics, parsedSiteID, vpcLifecycleEvents).Get(ctx, nil)
	if serr != nil {
		logger.Warn().Err(serr).Msg("failed to execute activity: RecordVpcLifecycleMetrics")
	}
