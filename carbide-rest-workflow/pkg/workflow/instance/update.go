...
func UpdateInstanceInventory(ctx workflow.Context, siteID string, instanceInventory *cwssaws.InstanceInventory) (err error) {
	...
	// Trigger workflow to record instance lifecycle metrics
	serr := workflow.ExecuteActivity(ctx, instanceManager.RecordInstanceLifecycleMetrics, parsedSiteID, objectLifecycleEvents).Get(ctx, nil)
	if serr != nil {
		logger.Warn().Err(serr).Msg("failed to execute activity: RecordInstanceLifecycleMetrics")
	}
