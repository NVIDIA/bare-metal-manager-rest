...
func TestBuildInterface(t *testing.T, dbSession *cdb.Session, instanceID, subnetID *uuid.UUID, vpID *uuid.UUID, isPhysical bool, userID *uuid.UUID, status string) *cdbm.Interface {
	is := &cdbm.Interface{
		ID:          uuid.New(),
...
func TestAssertMetricExistsTimes(t *testing.T, reg *prometheus.Registry, metricName string, expectedCount int, expectedLabels map[string]string, expectedDuration time.Duration) {
	...
	// Check duration for summary metrics
	if expectedDuration > 0 && len(foundMetricFamily.Metric) > 0 {
		metric := foundMetricFamily.Metric[0]
		assert.NotNil(t, metric.Summary, "Expected summary metric for duration check")
		assert.Greater(t, *metric.Summary.SampleCount, uint64(0), "Expected at least one sample")

		// Summary metrics store the sum of all observations
		actualDuration := time.Duration(*metric.Summary.SampleSum * float64(time.Second))
