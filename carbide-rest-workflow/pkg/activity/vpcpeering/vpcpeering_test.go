	...
	for i := 0; i < 38; i++ {
		...
		if i < 34 {
			ctrlVpcPeering := &cwssaws.VpcPeering{
				Id:        &cwssaws.UUID{Value: vpcPeering.ID.String()},
				VpcId:     &cwssaws.UUID{Value: prev_vpc.ID.String()},
				PeerVpcId: &cwssaws.UUID{Value: curr_vpc.ID.String()},
			}
	...
	tests := []struct {
		...
		},
		{
			...
			args: args{
				...
				vpcPeeringInventory: &cwssaws.VPCPeeringInventory{
					VpcPeerings: []*cwssaws.VpcPeering{
						{Id: &cwssaws.UUID{Value: vp1.ID.String()}},
						{Id: &cwssaws.UUID{Value: vp2.ID.String()}},
						{Id: &cwssaws.UUID{Value: vp3.ID.String()}},
						{Id: &cwssaws.UUID{Value: vp4.ID.String()}},
					},
				},
			},
