...
func TestPotentialRaceCondition(t *testing.T) {
	tests := []struct {
		...
		{
			name:       "recent action within race window",
			actionTime: time.Now(),
			want:       true,
		},
		{
			name:       "action just outside race window",
			actionTime: time.Now().Add(-time.Duration(cwu.InventoryReceiptInterval + time.Second*6)),
			want:       false,
		},
		{
			name:       "old action well outside race window",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := util.IsTimeAPotentialRaceCondition(tt.actionTime); got != tt.want {
				t.Errorf("IsTimeAPotentialRaceCondition() = %v, want %v", got, tt.want)
			}
		})
	}
}
