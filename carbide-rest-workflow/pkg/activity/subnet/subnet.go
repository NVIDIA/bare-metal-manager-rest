...
// RecordSubnetLifecycleMetrics triggers workflow to record subnet lifecycle metrics
func (ms ManageSubnet) RecordSubnetLifecycleMetrics(ctx context.Context, siteID uuid.UUID, subnetLifecycleEvents []cwm.InventoryObjectLifecycleEvent) error {
	logger := log.With().Str("Activity", "RecordSubnetLifecycleMetrics").Str("Site ID", siteID.String()).Logger()

	logger.Info().Msg("starting activity")

	workflowOptions := client.StartWorkflowOptions{
		ID:        "subnet-lifecycle-metrics-record-" + uuid.NewString(),
		TaskQueue: queue.SiteTaskQueue,
	}

	// Use the `site` namespace client dedicated to this worker type
	we, serr := ms.tc.ExecuteWorkflow(ctx, workflowOptions, "RecordSubnetLifecycleMetrics",
		// Workflow arguments
		siteID.String(),
		subnetLifecycleEvents,
	)
	if serr != nil {
		logger.Error().Err(serr).Msg("failed to execute RecordSubnetLifecycleMetrics workflow")
	} else {
		logger.Info().Str("Workflow ID", we.GetID()).Msg("triggered Site workflow to record Subnet lifecycle metrics")
	}
	...
	for _, event := range subnetLifecycleEvents {
		...
		if event.Created != nil {
			...
			if readySD != nil && pendingSD != nil && readyStatusCount == 1 {
				...
				mslm.statusTransitionTime.WithLabelValues(siteName, cwm.InventoryOperationTypeCreate, cdbm.SubnetStatusPending, cdbm.SubnetStatusReady).Observe(dur.Seconds())
			}
			...
			if deletingSD != nil {
				...
				mslm.statusTransitionTime.WithLabelValues(siteName, cwm.InventoryOperationTypeDelete, cdbm.SubnetStatusDeleting, cdbm.SubnetStatusDeleted).Observe(dur.Seconds())
			}
		}
	}
	...
	lifecycleMetrics := ManageSubnetLifecycleMetrics{
		...
		statusTransitionTime: prometheus.NewSummaryVec(
			prometheus.SummaryOpts{
				...
				Name:      "subnet_operation_duration_seconds",
				Help:      "Duration of Subnet operations (create/update/delete)",
