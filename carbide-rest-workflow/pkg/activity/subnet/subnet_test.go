			...
			args: args{
				...
				subnetInfo: &cwsv1.SubnetInfo{
					...
					NetworkSegment: &cwsv1.NetworkSegment{
						Id:   &cwsv1.UUID{Value: uuid.New().String()},
						Name: sb1.Name,
					},
...
func TestManageSubnet_UpdateSubnetsInDB(t *testing.T) {
	...
	for i := 0; i < 34; i++ {
		ctrlSubnet := &cwssaws.NetworkSegment{
			Id:   &cwssaws.UUID{Value: pagedSubnets[i].ControllerNetworkSegmentID.String()},
			...
			args: args{
				...
				subnetInventory: &cwsv1.SubnetInventory{
					Segments: []*cwsv1.NetworkSegment{
						{
							Id:    &cwsv1.UUID{Value: subnet1.ControllerNetworkSegmentID.String()},
			...
			args: args{
				...
				subnetInventory: &cwsv1.SubnetInventory{
					Segments: []*cwsv1.NetworkSegment{
						...
						{
							Id:    &cwsv1.UUID{Value: subnet5.ControllerNetworkSegmentID.String()},
							Name:  subnet5.Name,
							State: cwsv1.TenantState_READY,
						},
						{
							Id:    &cwsv1.UUID{Value: subnet6.ControllerNetworkSegmentID.String()},
							Name:  subnet6.Name,
							State: cwsv1.TenantState_READY,
						},
						{
							Id:    &cwsv1.UUID{Value: uuid.NewString()},
...
func Test_SubnetMetrics_Create_PendingToReady(t *testing.T) {
	...
	util.TestAssertMetricExistsTimes(t, reg, "cloud_workflow_subnet_operation_duration_seconds", 1, map[string]string{
...
func Test_SubnetMetrics_Create_ReadyErrorReady(t *testing.T) {
	...
	util.TestAssertMetricExistsTimes(t, reg, "cloud_workflow_subnet_operation_duration_seconds", 0, nil, 0)
}
