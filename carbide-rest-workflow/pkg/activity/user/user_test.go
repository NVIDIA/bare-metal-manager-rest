...
func TestNewManageUser(t *testing.T) {
	t.Skip("Skipping test that requires fully initialized Config with viper instance")
	type args struct {
		dbSession *cdb.Session
		cfg       *config.Config
	}
	...
	dbSession := &cdb.Session{}
	cfg := &config.Config{} // Use empty config for testing instead of reading from environment
