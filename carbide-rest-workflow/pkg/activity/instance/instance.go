...
func (mi ManageInstance) CreateInstanceViaSiteAgent(ctx context.Context, instanceID uuid.UUID) error {
	...
	for _, ibinterface := range ibinterfaces {
		...
		ibcfg := &cwsv1.InstanceIBInterfaceConfig{
			...
			IbPartitionId:  &cwsv1.UUID{Value: ibinterface.InfiniBandPartition.ControllerIBPartitionID.String()},
		}
		...
		if controllerInstance.Config.Network != nil && controllerInstance.Status.Network != nil {
			...
			for idx, interfaceConfig := range controllerInstance.Config.Network.Interfaces {
				...
				if interfaceStatus != nil {
					...
					if interfaceStatus.DeviceInstance != nil {
...
// RecordInstanceLifecycleMetrics triggers workflow to record instance lifecycle metrics
func (mi ManageInstance) RecordInstanceLifecycleMetrics(ctx context.Context, siteID uuid.UUID, instanceLifecycleEvents []cwm.InventoryObjectLifecycleEvent) error {
	logger := log.With().Str("Activity", "RecordInstanceLifecycleMetrics").Str("Site ID", siteID.String()).Logger()

	logger.Info().Msg("starting activity")

	workflowOptions := client.StartWorkflowOptions{
		ID:        "instance-lifecycle-metrics-record-" + uuid.NewString(),
		TaskQueue: queue.SiteTaskQueue,
	}

	// Use the `site` namespace client dedicated to this worker type
	we, serr := mi.tc.ExecuteWorkflow(ctx, workflowOptions, "RecordInstanceLifecycleMetrics",
		// Workflow arguments
		siteID,
		instanceLifecycleEvents,
	)
	if serr != nil {
		logger.Error().Err(serr).Msg("failed to execute RecordInstanceLifecycleMetrics workflow")
	} else {
		logger.Info().Str("Workflow ID", we.GetID()).Msg("triggered Site workflow to record Instance lifecycle metrics")
	}
...
type ManageInstanceLifecycleMetrics struct {
	dbSession            *cdb.Session
	statusTransitionTime *prometheus.SummaryVec
	...
	for _, event := range instanceLifecycleEvents {
		...
		if event.Created != nil {
			...
			if readySD != nil && pendingSD != nil && readyStatusCount == 1 {
				...
				milm.statusTransitionTime.WithLabelValues(siteName, cwm.InventoryOperationTypeCreate, cdbm.InstanceStatusPending, cdbm.InstanceStatusReady).Observe(dur.Seconds())
			}
			...
			if terminatingSD != nil {
				...
				milm.statusTransitionTime.WithLabelValues(siteName, cwm.InventoryOperationTypeDelete, cdbm.InstanceStatusTerminating, cdbm.InstanceStatusTerminated).Observe(dur.Seconds())
			}
		}
	}
...
func NewManageInstanceLifecycleMetrics(reg prometheus.Registerer, dbSession *cdb.Session) ManageInstanceLifecycleMetrics {
	inventoryMetrics := ManageInstanceLifecycleMetrics{
		dbSession: dbSession,
		statusTransitionTime: prometheus.NewSummaryVec(
			prometheus.SummaryOpts{
				Namespace: cwm.MetricsNamespace,
				Name:      "instance_operation_duration_seconds",
				Help:      "Duration of instance operations (create/update/delete)",
