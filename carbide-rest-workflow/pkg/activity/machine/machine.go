...
func (mm *ManageMachine) UpdateMachinesInDB(ctx context.Context, siteIDStr string, machineInventory *cwssaws.MachineInventory) error {
	...
	for _, machineInfo := range machineInventory.Machines {
		...
		// Determine status
		machineStatus, statusMessage, isMachineAllocatable := getForgeMachineStatus(controllerMachine, slogger)
		...
		if discoveryInfo != nil && discoveryInfo.DmiData != nil {
			...
			if dmiData.ChassisSerial != "" {
				serialNumber = &dmiData.ChassisSerial
			}
		}
		...
		if !found {
			...
			createInput := cdbm.MachineCreateInput{
				...
				IsAllocatable:            isMachineAllocatable,
...
// Utility function to get Forge Machine status and allocatable from Controller Machine state
func getForgeMachineStatus(controllerMachine *cwssaws.Machine, logger zerolog.Logger) (string, string, bool) {
	...
	if controllerMachine.MaintenanceStartTime != nil {
		message := "Machine is in maintenance mode"
		if controllerMachine.MaintenanceReference != nil {
			message = fmt.Sprintf("%s: %s", message, *controllerMachine.MaintenanceReference)
		}
		return cdbm.MachineStatusMaintenance, message, false
	}

	// Determine if Machine have alerts
		...
		for _, alert := range controllerMachine.Health.Alerts {
			for _, clf := range alert.Classifications {
				if clf == MachinePreventAllocations {
					return cdbm.MachineStatusError, MachinePreventAllocationStatusMessage, false
				}
			}
		}
	}
	// Remove context data from the state
	...
	case controllerMachineStatePrefixCreated, controllerMachineStatePrefixHostInitializing, controllerMachineStatePrefixHostReprovisioning:
		return cdbm.MachineStatusInitializing, "Machine is initializing", true
	case controllerMachineStatePrefixDPUDiscovering, controllerMachineStatePrefixDPUInitializing, controllerMachineStatePrefixReprovisioning:
		return cdbm.MachineStatusInitializing, "Machine DPU is being configured", true
	case controllerMachineStatePrefixWaitingForCleanup:
		return cdbm.MachineStatusDecommissioned, "Machine is waiting for cleanup", true
	case controllerMachineStatePrefixMeasuring:
		if controllerMachineSubstate == controllerMachineMeasuringSubstateWaitingForMeasurements {
			return cdbm.MachineStatusInitializing, "System is waiting to receive attestation measurements from Machine", true
		} else if controllerMachineSubstate == controllerMachineMeasuringSubstatePendingBundle {
			return cdbm.MachineStatusInitializing, "Machine did not match any approved measurements. Requires automated or manual approval", true
		} else {
			return cdbm.MachineStatusInitializing, "Machine is undergoing measured boot attestation", true
		}
	...
	case controllerMachineStatePrefixBomValidating:
		if controllerMachineSubstate == controllerMachineBomValidatingSubstateMatchingSku {
			return cdbm.MachineStatusInitializing, "Machine is undergoing SKU matching", true
		} else if controllerMachineSubstate == controllerMachineBomValidatingSubstateUpdatingInventory {
			return cdbm.MachineStatusInitializing, "Machine inventory is being updated", true
		} else if controllerMachineSubstate == controllerMachineBomValidatingSubstateVerifyingSku {
			return cdbm.MachineStatusInitializing, "Machine SKU is being verified", false
		} else if controllerMachineSubstate == controllerMachineBomValidatingSubstateWaitingForSkuAssignment {
			return cdbm.MachineStatusInitializing, "Machine is awaiting SKU assignment", true
		} else if controllerMachineSubstate == controllerMachineBomValidatingSubstateSkuVerificationFailed {
			return cdbm.MachineStatusError, "Machine has failed SKU verification", false
		} else {
			return cdbm.MachineStatusInitializing, "Machine is undergoing BOM validation", true
		}
	case controllerMachineStatePrefixMachineValidation:
		return cdbm.MachineStatusInitializing, "Machine is undergoing machine validation", true
	case controllerMachineStatePrefixAssigned:
		return cdbm.MachineStatusInUse, "Machine is being used by an Instance", false
	case controllerMachineStatePrefixReady:
		return cdbm.MachineStatusReady, "Machine is ready for assignment", true
	case controllerMachineStatePrefixForceDeletion:
		return cdbm.MachineStatusDecommissioned, "Machine was decommissioned", true
	case controllerMachineStatePrefixFailed:
		if controllerMachineSubstate == controllerMachineFailedMeasurementsFailedSignatureCheck {
			return cdbm.MachineStatusError, "Machine measurements failed signature check", false
		} else if controllerMachineSubstate == controllerMachineFailedMeasurementsRetired {
			return cdbm.MachineStatusError, "Machine matched retired measurements", false
		} else if controllerMachineSubstate == controllerMachineFailedMeasurementsRevoked {
			return cdbm.MachineStatusError, "Machine matched revoked measurements", false
		} else if controllerMachineSubstate == controllerMachineFailedMachineValidation {
			return cdbm.MachineStatusError, "Machine has failed machine validation", false
		} else {
			errMsg := "Machine has encountered a failure"
			if controllerMachineSubstate != "" {
				errMsg = fmt.Sprintf("%s: %s", errMsg, controllerMachineSubstate)
			}
			return cdbm.MachineStatusError, errMsg, false
		}
	case controllerMachineStateMissing:
		return cdbm.MachineStatusError, "Machine is missing on Site", false
	default:
