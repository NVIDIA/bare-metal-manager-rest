...
// RecordVpcLifecycleMetrics triggers workflow to record VPC lifecycle metrics
func (mv ManageVpc) RecordVpcLifecycleMetrics(ctx context.Context, siteID uuid.UUID, vpcLifecycleEvents []cwm.InventoryObjectLifecycleEvent) error {
	logger := log.With().Str("Activity", "RecordVpcLifecycleMetrics").Str("Site ID", siteID.String()).Logger()

	logger.Info().Msg("starting activity")

	// Record latency for this inventory call
	workflowOptions := client.StartWorkflowOptions{
		ID:        "vpc-lifecycle-metrics-record-" + uuid.NewString(),
		TaskQueue: queue.SiteTaskQueue,
	}

	// Use the `site` namespace client dedicated to this worker type
	we, serr := mv.tc.ExecuteWorkflow(ctx, workflowOptions, "RecordVpcLifecycleMetrics",
		// Workflow arguments
		siteID.String(),
		vpcLifecycleEvents,
	)
	...
	if serr != nil {
		logger.Error().Err(serr).Msg("failed to execute RecordVpcLifecycleMetrics workflow")
	} else {
		logger.Info().Str("Workflow ID", we.GetID()).Msg("triggered Site workflow to record VPC lifecycle metrics")
	}
...
func (mv ManageVpc) UpdateVpcMetadata(ctx context.Context, siteID uuid.UUID, tc client.Client, vpcID uuid.UUID, controllerVpc *cwssaws.Vpc) error {
	...
	updateVpcRequest := &cwssaws.VpcUpdateRequest{
		Id: &cwssaws.UUID{Value: vpc.ID.String()},
...
func (mvlm ManageVpcLifecycleMetrics) RecordVpcStatusTransitionMetrics(ctx context.Context, siteID uuid.UUID, vpcLifecycleEvents []cwm.InventoryObjectLifecycleEvent) error {
	logger := log.With().Str("Activity", "RecordVpcStatusTransitionMetrics").Logger()
	...
	for _, event := range vpcLifecycleEvents {
		...
		if event.Created != nil {
			...
			if deletingStatusDetail != nil {
				...
				mvlm.statusTransitionTime.WithLabelValues(siteName, cwm.InventoryOperationTypeDelete, cdbm.VpcStatusDeleting, "Deleted").Observe(duration.Seconds())
			}
		}
	}
	...
	lifecycleMetrics := ManageVpcLifecycleMetrics{
		...
		statusTransitionTime: prometheus.NewSummaryVec(
			prometheus.SummaryOpts{
				...
				Name:      "vpc_operation_duration_seconds",
				Help:      "Duration of VPC operations (create/update/delete)",
				Objectives: map[float64]float64{
					0.9:  0.01, // 90th percentile ± 1%
					0.95: 0.01, // 95th percentile ± 1%
					0.99: 0.01, // 99th percentile ± 1%
				},
			},
